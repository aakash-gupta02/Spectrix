    ...
    return (
        <div className="space-y-2">
            {items.map((item) => {
                ...
                return (
                    <div key={key} className="border border-border bg-surface-2 px-3 py-2.5">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                    <span className={`inline-flex border px-2 py-0.5 text-[0.625rem] uppercase tracking-[0.12em] ${methodToneClasses[method] || "border-border bg-white/5 text-body"}`}>
                                        {method || "API"}
                                    </span>
                                    <span className="truncate text-xs text-heading">{item?.endpointName || "Unnamed endpoint"}</span>
                                </div>
                                <p className="mt-1 truncate text-[0.6875rem] text-body">{item?.path || "-"}</p>
                            </div>

                            <div className="shrink-0 text-right">
                                <p className="text-sm font-light tracking-tight text-heading">{formatCount(item?.count)}</p>
