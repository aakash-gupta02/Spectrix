...
export default function DeleteServiceModal({ isOpen, service, onClose, onDeleted }) {
  ...
  const deleteServiceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onDeleted?.("Service deleted successfully.");
      onClose();
    },
  });
  ...
  useEffect(() => {
    ...
    const handleEscape = (event) => {
      if (event.key === "Escape" && !deleteServiceMutation.isPending) {
        onClose();
      }
    };
      ...
      >
        ...
        <div className="space-y-4 px-5 py-4 text-sm text-body">
          <p>
            ...
            <span className="mx-1 text-heading">{service.name || "this service"}</span>
            from your workspace.
          </p>
          ...
          {deleteServiceMutation.isError ? (
            <div className="border border-red-500/40 bg-red-500/5 px-3 py-2 text-sm text-red-300">
              {deleteServiceMutation.error?.response?.data?.message ||
